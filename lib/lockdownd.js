...
const LOCKDOWN_PROTOCOL_VERSION = "2";
...
class Client extends PropertyListService {
	...
	pair() {
		const _this = this;
		return meaco(function* () {
			const pairRecord = yield _this.__usbmuxdClient.readPairRecord(_this.__udid);
			
			const request =  _this.__createRequest('Pair')
			request.PairRecord = pairRecord;
			request.PairingOptions = {
				ExtendedPairingErrors: true,
			};
			request.ProtocolVersion = LOCKDOWN_PROTOCOL_VERSION;
			const response = yield _this._writeXml(request);
			return response;
		});
	}
